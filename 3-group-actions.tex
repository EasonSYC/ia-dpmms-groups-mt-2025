\section{Group Actions}
\label{sec:group-actions}

We return to the aspect where we started, that groups describe symmetries. Groups become groups of symmetry when they \textbf{act}.

\subsection{Group Actions and Homomorphisms}
\label{sec:group-actions-homomorphisms}

\begin{definition}[Group Action]
    \label{def:group-action}%
    An \emph{action} of a group \(G\) on a set \(X\) is a map
    \[
        \alpha: G \times X \to X, \para{g, x} \mapsto \alpha\para{g, x} = gx
    \]
    satisfying
    \begin{enumerate}
        \item \(\alpha\para{e, x} = ex = x\) for all \(x \in X\): the identity acts as an identity;
        \item \(\alpha\para{g \cdot h, x} = \alpha{g, \alpha\para{h, x}}\), or \(\para{g \cdot h}x = g \para{hx}\) for all \(g, h \in G\) and \(x \in X\): composition is compatible.
    \end{enumerate}

    We write \(G \actson X\) to indicate that \(G\) acts on \(X\).
\end{definition}

\begin{examples}[Examples of Group Actions]
    \begin{enumerate}
        \item For any group \(G\) and set \(X\), the action
        \[
            gx = x
        \]
        is the \emph{trivial action}.
        \item \(\Sym\para{X}\) acts on \(X\) for any set \(X\) by
        \[
            fx = f\para{x}.
        \]
        \item If \(G \actson X\) and \(H \subgroup G\), then \(H \actson X\) by restriction.
        \item In particular, \(\Isom\para{\CC} \subgroup \Sym\para{\CC}\) acts on \(\CC\).
        \item Similarly, \(D_{2n}\) acts on \(X_n\), the regular \(n\)-gon. It acts on the set of vertices
        \[
            \set{z \in \CC}{z^n = 1}.
        \]
        \item Every group \(G\) acts on \emph{itself} by left-multiplication: let \(X = G\), and \(G \actson G\) by
        \[
            g\gamma = g \cdot \gamma
        \]
        for \(g \in G\), \(\gamma \in X = G\). This is the \emph{left regular action}.
    \end{enumerate}
\end{examples}

We may think of group actions in another useful way.

\begin{theorem}[Actions Define Homomorphism]
    \label{thm:action-homomorphism}%
    An action of a group \(G\) on a set \(X\) is the same as a homomorphism
    \[
        \phi: G \to \Sym\para{X}.
    \]
\end{theorem}

\begin{proof}
    Suppose \(G \actson X\). Consider the map
    \[
        t_g: X \to X, x \mapsto gx
    \]
    for any \(g \in G\). Now,
    \begin{align*}
        t_{g\inverse} \para{t_g x} &= t_{g\inverse} \para{gx} \\
        &= g\inverse \para{gx} \\
        &= \para{g\inverse g} x \\
        &= ex \\
        &= x
    \end{align*}
    so \(t_{g\inverse} \circ t_g = \identity_{X}\).

    Therefore, \(t_{g\inverse}\) is \(\para{t_g}\inverse\) and hence \(t_g\) is a bijection.

    This tells us that \(t_g\) is a permutation on \(X\),
    \[
        t_g \in \Sym\para{X}.
    \]

    Therefore, we define
    \[
        \phi: G \to \Sym\para{X}, g \mapsto t_{g},
    \]
    and we prove that \(\phi\) is a homomorphism. Take \(g, h \in G\), we have
    \[
        \phi\para{g} \phi\para{h} = t_g \circ t_h,
    \]
    so for all \(x \in X\),
    \begin{align*}
        \phi\para{g} \phi\para{h} \para{x} &= t_g \circ t_h \para{x} \\
        &= t_g \para{t_h \para{x}} \\
        &= t_g \para{hx} \\
        &= g \para{hx} \\
        &= \para{gh} x \\
        &= t_{gh} \para{x} \\
        &= \phi\para{gh} \para{x},
    \end{align*}
    and hence, \(\phi\para{g} \phi\para{h} = \phi\para{gh}\).

    Therefore, \(\phi\) is a homomorphism as required.

    Conversely, given a homomorphism
    \[
        \phi: G \to \Sym\para{X}
    \]
    we may define an action \(G \actson X\) by \(gx = \phi\para{g} x\). We check this is indeed an action:
    \begin{enumerate}
        \item Identity: \(ex = \phi\para{e} \para{x} = \identity_{X} \para{x} = x\).
        \item Compatibility:
        \begin{align*}
            \para{gh} x &= \phi\para{gh} \para{x} \\
            &= \phi\para{g} \phi\para{h} \para{x} \\
            &= \phi\para{g} \para{\phi\para{h} \para{x}} \\
            &= \phi\para{g} \para{hx} \\
            &= g \para{hx}.
        \end{align*}
    \end{enumerate}
\end{proof}

The left regular action allows us to think of groups as subgroups of symmetric groups.

\begin{theorem}[Cayley's Theorem]
    \label{thm:cayley}%
    Every group \(G\) is isomorphic to a subgroup to some \(\Sym\para{X}\). Furthermore, if \(\abs{G} < \infty\), we may choose \(X\) to be \(\abs{X} < \infty\), or even stronger, \(\abs{X} \leq \abs{G}\).
\end{theorem}

\begin{proof}
    We choose in particular \(X = G\), and the left regular action gives \(G \actson X = G\). By \autoref{thm:action-homomorphism}, this is equivalent to a homomorphism
    \[
        \phi: G \to \Sym\para{X}.
    \]

    Let \(H = \img \para{\phi} \subgroup \Sym\para{X}\) be a subgroup. Then we may think of \(\phi\) as a \emph{surjective} homomorphism
    \[
        \phi: G \to H,
    \]
    and we claim that this is injective. Indeed, take \(g \in \ker\para{\phi}\), then \(\phi\para{g} = \identity_{X}\), and therefore
    \[
        g \gamma = \gamma
    \]
    for all \(\gamma \in G\). In particular, taking \(\gamma = e \in G\) gives \(g = e\).

    Therefore, the kernel is trivial, and \(\phi\) is an injection. Therefore, \(\phi\) is a bijection,
    \[
        G \isomorphic H \subgroup \Sym\para{X},
    \]
    as required.

    Furthermore, notice \(X = G\), so \(\abs{G} < \infty\) allows us to choose \(\abs{X} < \infty\).
\end{proof}

\subsection{Orbits and Stabilisers}
\label{sec:orbits-and-stabilisers}

We in fact can learn a huge amount about groups by thinking about group actions.

\begin{definition}[Orbits and Stabilisers]
    \label{def:orbit-stabiliser}%
    Let \(G \actson X\) and let \(x \in X\).
    \begin{enumerate}
        \item The \emph{orbit} of \(x\) is the set
        \[
            \orb_{G}\para{x} = Gx = \set{y \in X}{\exists g \in G, y = gx}.
        \]
        \item The \emph{stabiliser} of \(x\) is the set
        \[
            \stab_{G}\para{x} = \set{g \in G}{g\para{x} = x}.
        \]
    \end{enumerate}

    Note some authors use \(G_x\) for stabilisers, where \(x\) is a subscript.

    \begin{itemize}
        \item If \(\orb_{G}\para{x} = X\), we say the action is \emph{transitive}. (We will see soon that this statement is equivalent with for \emph{all} \(x \in X\) or for \emph{some} \(x \in X\).)
        \item If every element \(g \in G\) (except the identity \(g = e\)) has an \(x \in X\) such that \(gx \neq x\), then we say the action is \emph{faithful}.
    \end{itemize}
\end{definition}

\begin{remark}
    An action \(G \actson X\) is faithful is equivalent to that the associated homomorphism has a trivial kernel/is injective.
\end{remark}

\begin{proposition}[Stabilisers are Subgroups, Orbits Partition]
    \label{prop:stabilisers-subgroups-orbits-partition}%
    Suppose \(G \actson X\). Then
    \begin{enumerate}
        \item \(\stab_{G} \para{x} \subgroup G\) for any \(x \in X\).
        \item The orbits
        \[
            \set{\orb_{G}\para{y}}{y \in X}
        \]
        form a partition of \(X\).
    \end{enumerate}
\end{proposition}

\begin{remark}
    The second part of \autoref{prop:stabilisers-subgroups-orbits-partition} tells us that if \(\orb\para{x} = X\) for some \(x \in X\), then there is only one orbit, and hence \(\orb\para{x} = X\) for all \(x \in X\).
\end{remark}

\begin{proof}
    \begin{enumerate}
        \item We check that \(\stab_{G} \para{x}\) satisfies \autoref{def:subgroup} (the definition of a subgroup). If \(g, h \in \stab_{G}\para{x}\), then
        \begin{align*}
            \para{gh} x &= g\para{h\para{x}} && \text{\autoref{def:group-action} (the definition of a group action)} \\
            &= g\para{x} && h \in \stab_{G} \para{x} \\
            &= x, && g \in \stab_{G} \para{x}
        \end{align*}
        so \(gh \in \stab_{G} \para{x}\).

        Also, \(ex = x\) from \autoref{def:group-action} (the definition of a group action), so \(e \in \stab_G \para{x}\).

        Finally, fir \(g \in \stab_G \para{x}\),
        \begin{align*}
            g\inverse x &= g\inverse \para{gx} && g \in \stab_{G} \para{x} \\
            &= \para{g\inverse g} x && \text{\autoref{def:group-action} (the definition of a group action)} \\
            &= ex \\
            &= x, && \text{\autoref{def:group-action} (the definition of a group action)}
        \end{align*}
        so \(g\inverse \in \stab_{G} \para{x}\).

        Hence, \(\stab_{G} \para{x} \subgroup G\).

        \item The proof is similar to the proof of \autoref{lem:cosets-partition} (cosets partition).
        \begin{itemize}
            \item For any \(x \in X\), \(x = ex\) for \(e \in G\), so \(x \in \orb_G \para{x}\), so indeed orbits cover \(X\).
            \item If \(\orb_{G} \para{x_1} \cap \orb_{G} \para{x_2} \neq \emptyset\), then there is \(y = g_1 x_1 = g_2 x_2\) for some \(g_1, g_2 \in G\).
        
            Hence,
            \begin{align*}
                x_1 &= \para{g_1 \inverse g_1} x_1 \\
                &= g_1 \inverse \para{g_1 x_1} \\
                &= g_1 \inverse \para{g_2 x_2} \\
                &= \para{g_1 \inverse g_2} x_2 \\
                &\in \orb_{G} \para{x_2}.
            \end{align*}

            Moreover, any \(g x_1\) can be written as
            \[
                g x_1 = g \para{g_1\inverse g_2} x_2 \in \orb_{G} \para{x_2}.
            \]

            Hence, \(\orb_{G} \para{x_1} \subseteq \orb_{G} \para{x_2}\). Likewise, \(\orb_{G} \para{x_2} \subseteq \orb_{G} \para{x_1}\), so we have
            \[
                \orb_{G} \para{x_1} = \orb_{G} \para{x_2}
            \]
            as desired.
        \end{itemize}
    \end{enumerate}
\end{proof}

\begin{example}[\(D_{2n} \actson X_n\), the regular \(n\)-gon]
    As in \autoref{fig:dihedral-group-action-hexagon} for the hexagon case, for any \(j\),
    \begin{align*}
        r^j s \para{x} &= r^j \para{x} \\
        &= e^{\frac{2 \pi i j}{n}} \cdot 1 \\
        &= e^{\frac{2 \pi i j}{n}},
    \end{align*}
    so
    \begin{align*}
        \orb_{D_{2n}}\para{x} &= \set{e^{\frac{2 \pi i j}{n}}}{0 \leq j < n} \\
        &= \brce{n\text{-th roots of unity}}.
    \end{align*}

    The above calculation also shows that
    \[
        gx = x \iff g \in \brce{r, s},
    \]
    so
    \[
        \stab_{D_{2n}}\para{x} = \brce{r, s}.
    \]
\end{example}

\begin{figure}[ht!]
    \centering
    \caption{\(D_{2n} \actson X_n\) for \(n = 6\), the regular hexagon}
    \label{fig:dihedral-group-action-hexagon}
\end{figure}

\begin{theorem}[Orbit--Stabiliser]
    \label{thm:orbit-stabiliser}%
    Suppose \(G \actson X\). Let \(x \in X\), the map
    \[
        g \stab_{G}\para{x} \mapsto gx
    \]
    defines a well-defined bijection
    \[
        G / \stab_{G}\para{x} \to \orb_{G}\para{x}.
    \]
\end{theorem}

We first prove a corollary of this theorem, naming the counting version of this.

\begin{corollary}[Orbit--Stabiliser]
    \label{cor:orbit-stabiliser}%
    If \(G \actson X\) and \(x \in X\), then
    \[
        \abs{G} = \abs{\orb_{G}\para{x}} \abs{\stab_{G}\para{x}}.
    \]
\end{corollary}

\begin{proof}
    \autoref{thm:orbit-stabiliser} (Orbit--Stabiliser Theorem) gives
    \[
        \abs{\orb_{G} \para{x}} = \abs{G / \stab_{G} \para{x}},
    \]
    and \autoref{thm:lagrange} (Lagrange's Theorem) gives
    \[
        \abs{G / \stab_{G} \para{x}} = \abs{G} / \abs{\stab_{G} \para{x}}.
    \]

    This gives the result as desired.
\end{proof}

Before we prove \autoref{thm:orbit-stabiliser} (Orbit--Stabiliser Theorem), we first give an example of the counting version of this.

\begin{example}[\(D_{2n} \actson X_n\)]
    We see that if \(x \in X_n\) is a vertex, then
    \[
        \abs{\orb_{D_{2n}} \para{x}} = n
    \]
    and
    \[
        \abs{\stab_{D_{2n}} \para{x}} = 2.
    \]

    This gives an easy (but circular) 'proof' that
    \[
        \abs{D_{2n}} = 2n.
    \]
\end{example}

\begin{proof}[of \autoref{thm:orbit-stabiliser} (Orbit--Stabiliser Theorem)]
    We write
    \[
        S = \stab_{G} \para{x},
    \]
    and define
    \[
        \phi \para{gS} = gx.
    \]

    We check that \(\phi\) is a well-defined bijection.

    \begin{itemize}
        \item \textbf{\(\phi\) is well-defined.} Let \(g_1, g_2 \in G\) such that \(g_1 S = g_2 S\). We need to check that \(\phi\para{g_1 S} = \phi\para{g_2 S}\). Now, \(g_1 S = g_2 S\) means that
        \[
            g_1 = g_2 s
        \]
        for some \(s \in S\).

        Hence,
        \begin{align*}
            g_1 x &= \para{g_2 s} x \\
            &= g_2 \para{sx} \\
            &= g_2 x
        \end{align*}
        since \(s \in S\) gives \(sx = x\), as required.

        \item \textbf{\(\phi\) is surjective.} For any \(gx \in \orb_{G} \para{x}\), \(g \in G\), we note
        \[
            \phi\para{gS} = gx
        \]
        so \(\phi\) is surjective.

        \item \textbf{\(\phi\) is injective.} Suppose that \(\phi\para{g_1 S} = \phi\para{g_2 S}\), which is equivalent to \(g_1 x = g_2 x\). We want to show \(g_1 S = g_2 S\). Indeed, let \(g_1 = g_2 s\) for some \(s\), we want to show \(s \in S\):
        \begin{align*}
            sx &= \para{g_2 \inverse g_1} x \\
            &= g_2 \inverse \para{g_1 x} \\
            &= g_2 \inverse \para{g_2 x} \\
            &= \para{g_2 \inverse g_2} x \\
            &= ex \\
            &= x,
        \end{align*}
        so \(s \in \stab_{G} \para{x}\), and hence \(s \in S\).

        Therefore,
        \begin{align*}
            g_1 &= \para{g_2 g_2\inverse} g_1 \\
            &= g_2 \para{g_2\inverse g_1} \\
            &= g_2 s \\
            &= g_2 S,
        \end{align*}
        and since cosets partition, it follows that \(g_1 S = g_2 S\) as required.
    \end{itemize}
\end{proof}

\begin{example}[Symmetries of a Cube]
    Let \(G\) be the group of isometries of a cube.

    As in \autoref{fig:symmetries-of-a-cube}, let \(x\) be the centre of a face of a cube. Then
    \[
        \abs{\orb_{G} \para{x}} = 6,
    \]
    while
    \[
        \stab_{G}\para{x} \isomorphic D_8,
    \]
    so
    \[
       \abs{\stab_{G}\para{x}} = 8.
    \]

    Therefore,
    \begin{align*}
        \abs{G} &= \abs{\orb_{G} \para{x}} \abs{\stab_{G} \para{x}} \\
        &= 6 \cdot 8 \\
        &= 48.
    \end{align*}
\end{example}

\begin{figure}[ht!]
    \centering
    \caption{A Cube and the Centre of a Face}
    \label{fig:symmetries-of-a-cube}
\end{figure}

The next theorem is a different kind of application of \autoref{cor:orbit-stabiliser} (Orbit--Stabiliser Theorem). The proof is not very intuitive, but gives a very important result in group theory.

\begin{theorem}[Cauchy's Theorem]
    \label{thm:cauchy}
    If \(\abs{G} < \infinity\), and \(p\) is a prime that divides \(\abs{G}\), then there is a \(g \in G\) such that
    \[
        \ord\para{g} = p.
    \]
\end{theorem}

\begin{proof}
    Consider the set \(X\) of \(p\)-tuples
    \[
        X = \set{\para{g_1, \ldots, g_p}}{g_i \in G, g_1 g_2 \cdots g_p = e}.
    \]

    We define an action of \(C_p\) on \(X\), where
    \[
        C_p = \brce{1, t, t^2, \ldots, t^{p - 1}} = \ang{t}
    \]
    as follows. Let
    \[
        t^k \para{g_1, \ldots, g_p} = \para{g_{k + 1}, \ldots, g_p, g_1, \ldots, g_k}.
    \]

    It is easy to see that
    \[
        \para{t^k \cdot t^l} \para{g_1, \ldots, g_p} = \para{t^k} \para{t^l \para{g_1, \ldots, g_p}}.
    \]

    We also need to check that
    \[
        \para{g_{k + 1}, \ldots, g_p, g_1, \ldots, g_k} \in X.
    \]

    Suppose that
    \[
        a = g_1 \cdots g_k, b = g_{k + 1} \cdots g_p,
    \]
    we have \(ab = e\), and hence \(b = a\inverse\), therefore
    \[
        ba = g_{k + 1} \cdots g_p g_1 \cdots g_k = e
    \]
    as required. This means we indeed defined an action \(C_p \actson X\).

    We now compute the size of \(X\). We may decide \(g_1, \ldots, g_{p - 1} \in G\) to be arbitrary elements, then \(g_p\) has to be determined uniquely, namely
    \[
        g_p = g_{p - 1}\inverse \cdots g_2\inverse g_1\inverse
    \]
    such that
    \[
        \para{g_1, \ldots, g_{p - 1}, g_p} \in X.
    \]

    Therefore,
    \[
        \abs{X} = \abs{G}^{p - 1}.
    \]

    From \autoref{prop:stabilisers-subgroups-orbits-partition}, \(C_p \actson X\) partition \(X\) into orbits
    \[
        X = \orb\para{x_1} \cup \orb\para{x_2} \cup \cdots \cup \orb\para{x_k}.
    \]

    Hence, by \autoref{cor:orbit-stabiliser} (Orbit--Stabiliser Theorem), for each \(j = 1, 2, \ldots, k\),
    \[
        \abs{\orb\para{x_j}} \divides \abs{C_p} = p
    \]
    so either the size of \(\orb\para{x_j}\) is \(1\), or \(p\).

    Let \(l\) be the number of orbits with size \(1\). After re-numbering, we may assume that
    \[
        \abs{\orb\para{x_j}} = \begin{cases}
            1, & 1 \leq j \leq l, \\
            p, & j > l.
        \end{cases}
    \]

    Since the orbits partition \(X\),
    \begin{align*}
        \abs{G}^{p - 1} &= \abs{X} \\
        &= \sum_{j = 1}^{k} \abs{\orb\para{x_j}} \\
        &= \sum_{j = 1}^{l} \abs{\orb{\para{x_j}}} + \sum_{j = l + 1}^{k} \abs{\orb{\para{x_j}}} \\
        &= l + p \cdot \para{k - l}.
    \end{align*}

    Since \(p \divides \abs{G}\), it follows that \(p \divides l\).

    Now, note that \(x = \para{g_1, \ldots, g_p}\) has an orbit of size \(1\) if and only if \(g_1 = g_2 = \cdots = g_p\).

    In particular, there is at least one such element giving such an orbit, namely
    \[
        \para{e, e, \ldots, e} \in X.
    \]

    So \(l > 0\). Since \(p \divides l\), it follows that
    \[
        l \geq p > 1,
    \]
    so there is at least \(1\) more orbit
    \[
        \para{g, g, \ldots, g} \in X
    \]
    with \(g \neq e\).

    The definition of \(X\) implies that \(g^p = e\), and hence we can see \(\ord\para{g} \divides p\), and hence \(\ord\para{g} = p\) since \(p\) is prime and \(g \neq e\), as required.
\end{proof}

This is a proof that is not very intuitive, but the result is very useful.

\subsection{Conjugation}
\label{sec:conjugation}

\begin{definition}[Conjugate, Conjugacy Class]
    \label{def:conjugate-ccl}%
    Let \(G\) be a group, and \(g \in G\). For any \(h \in G\), the element \(hgh\inverse \in G\) is called \emph{the conjugate of \(g\) by \(h\)}.

    The \emph{conjugacy class} of \(g \in G\) is
    \[
        \ccl_{G}\para{g} = \set{hgh\inverse}{h \in G}.
    \]
\end{definition}

The intuition behind this definition is that \(hgh\inverse\) has the same 'shape' as \(g\), and \(hgh\inverse\) corresponds to a 'change in coordinates' of \(g\) by \(h\). Conjugacy classes partition the group.

\begin{example}[Conjugacy in an Abelian Group]
    If \(G\) is abelian, then for any \(g, h \in G\),
    \[
        hgh\inverse = ghh\inverse = ge = g,
    \]
    so the only element conjugate to \(g\) is \(g\) itself. All conjugacy classes have size \(1\). (This condition is equivalent to a group being abelian.)
\end{example}

Conjugacy classes of reflections in a dihedral group demonstrates the intuition of 'size' well.

\begin{example}[Conjugacy Classes of Reflections in Dihedral Group]
    In Sheet 2, Q6, \(D_{2n}\) has \(1\) conjugacy class of reflections if \(n\) is odd, and \(2\) if \(n\) is even.
    
    As in \autoref{fig:ccl-dihedral}, when \(n = 6\), there are two 'types' of reflections, those whose axis run through vertices, and those who run through midpoints of edges. When \(n = 5\), there is only one 'type' of reflection: their axis all run through a vertex and a midpoint of an edge.
\end{example}

\begin{figure}[ht!]
    \centering
    \caption{Conjugacy in Dihedral Groups}
    \label{fig:ccl-dihedral}
\end{figure}

Conjugation defines another action of \(G \actson G\):
\[
    g \star \gamma = g \gamma g\inverse
\]
for \(g \in G, \gamma \in X = G\). This is \textbf{very different} to the left regular action. In terms of language in actions,
\[
    \ccl_{G} \para{g} = \orb_{G\star} \para{g}.
\]

From the example on abelian groups, we see that conjugating to an element itself is closely related to commutativity:
\[
    hgh\inverse = g \iff hg = gh.
\]

This leads to \autoref{def:centraliser-centre}.

\begin{definition}[Centraliser, Centre]
    \label{def:centraliser-centre}%
    The \emph{centraliser} of \(g \in G\) is
    \[
        C_G\para{g} = \set{h \in G}{h g h\inverse = g}.
    \]

    It is the elements that \emph{commute with \(g\)}.

    The \emph{centre} of \(G\) is
    \[
        Z\para{G} = \set{h \in G}{\forall g \in G, hg = gh} = \bigcap_{g \in G} C_G \para{g}.
    \]

    It is the elements that \emph{commute with everything in \(G\)}.
\end{definition}

Using the language of the conjugation action, we have
\[
    C_G\para{g} = \stab_{G\star} \para{g}
\]
and hence \(C_G\para{g} \subgroup G\) forms a group. Since \(Z\para{G}\) is an intersection of subgroups, it is itself a subgroup: \(Z\para{G} \subgroup G\).

\begin{remark}
    Another useful way to think of the centre is
    \[
        Z\para{G} = \bigcup_{g \in G, \abs{\ccl\para{g}} = 1} \ccl\para{g} = \bigcup_{g \in G, \abs{\ccl\para{g}} = 1} \brce{g},
    \]
    i.e. the union of the conjugacy classes with size one. (We will see later that this means \(Z\para{G} \normalsub G\).)
\end{remark}