...
\begin{definition}[M\"{o}bius Transformation]
    ...
    Let \(a, b, c, d \in \CC\), and \(ad - bc \neq 0\). We define the corresponding \emph{M\"{o}bius Transformation} \(\mu: \CC\compactify \to \CC\compactify\) as follows.

    \begin{itemize}
        \item If \(c \neq 0\), then
        \[
            \mu: z \mapsto \begin{cases}
...
\begin{proof}
    ...
    \[
        \identity_{\CC\compactify}: z \mapsto z = \frac{1 \cdot z + 0}{0 \cdot z + 1}
    \]
    is the identity.

    For \(\mu: z \mapsto \frac{az + b}{cz + d}\), let \(\nu: z \mapsto \frac{dz - b}{-cz + a}\), then
    \[
...
\begin{remark}
    The previous observation with matrix multiplication shows that the na\"{i}ve mapping is a surjective homomorphism \(\phi: \GL_{2} \para{\CC} \to \mobius\). In fact, we see that the restriction \(\rightbar{\phi}_{\SL_{2} \para{\CC}}: \SL_{2} \para{\CC} \to \mobius\) is surjective as well, and the kernels are
    \[
...
\begin{definition}[Fixed Point]
    ...
    Suppose \(f: X \to X\) is a permutation. Then any \(x \in X\) such that \(f\para{x} = x\) is called a \emph{fixed point} of \(f\). The set of all fixed points is denoted as
    \[
        \Fix\para{f} = \set{x \in X}{f\para{x} = x}.
    \]
\end{definition}

Recall that for \(\Isom\para{\CC}\), there is \autoref{lem:three-point-lemma} (the three--point lemma). There is a similar lemma for the M\"{o}bius group.
...
\begin{proof}
    Let
    \[
        \alpha \in \mobius: z \mapsto \frac{z - z_1}{z - z_3} \cdot \frac{z_2 - z_3}{z_2 - z_1},
    \]
    and we observe that
    \begin{align*}
        z_1 & \mapsto 0, \\
...
From \autoref{lem:three-point-mobius} (the three--point lemma for M\"{o}bius group), we see that the \(\mu\) given by \autoref{lem:triple-transitive} is \emph{unique}. We say the action \(\mobius \actson \CC\compactify\) is \textbf{sharply triply transitive}.
...
Similar to what we did in dihedral groups, we may use \autoref{lem:three-point-mobius} (the three--point lemma for M\"{o}bius group) to find a generating set for \(\mobius\).
...
\begin{proposition}[Generating Set for \(\mobius\)]
    ...
    \begin{enumerate}
        \item \(\alpha_{a}: z \mapsto az\) for \(a \neq 0, a \in \CC\);
        \item \(\beta_{b}: z \mapsto z + b\) for \(b \in \CC\);
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            \infty &\mapsto z_3,
        \end{align*}
        which are the same as \(\mu\). Therefore, by \autoref{lem:three-point-mobius} (the three--point lemma for M\"{o}bius group), we have
        \[
