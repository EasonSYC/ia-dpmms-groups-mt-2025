...
Let \(\phi: G \to H\) be a homomorphism, then \(\ker \phi \subgroup G\) is always a special kind of subgroup.
...
\begin{examples}[Examples of Normal Subgroups]
    \begin{enumerate}
        ...
        \[
            s r^k s\inverse = r^{-k} \in \ang{r}
        \]
        by \autoref{lem:dihedral-relation} (the dihedral relation). But \(\ang{s}\) is \emph{not} normal since
        \[
            r s r \inverse = s r^{-2} \notin \ang{s}.
        \]
        \item Suppose \(\phi: G \to G'\) is a homomorphism. If \(h \in \ker \phi\), \(g \in G\), then
        \[
...
\begin{remark}
    ...
    \[
        A \isomorphic B \times C \implies A / B \isomorphic C, A / C \isomorphic B.
    \]
...
\begin{proof}
    ...
    \[
        \bar{\phi}: G / \ker \phi \to \img \phi, g \ker \phi \mapsto \phi \para{g}.
    \]
...
\begin{examples}[Applications of \autoref{thm:first-isomorphism} (first isomorphism theorem)]
    \begin{enumerate}
        ...
        \item Similarly,
        \[
            \phi: \RR \to \CC\multiplicative, t \mapsto e^{2 \pi i t}
        \]
...
\begin{example}[Simple Abelian Groups]
    ...
    If \(G\) is an infinite abelian group and \(e \neq x \in G\) has infinite order, then \(\ang{x^2} \subgroup G\) is a proper subgroup; if \(x\) has finite order, then \(\ang{x}\) is a proper subgroup. Therefore, there are no infinite simple abelian groups. 
\end{example}
